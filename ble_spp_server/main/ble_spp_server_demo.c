...
#define PHY 1*10e6
#define packetbits 16
#include <time.h>
//Our code---------------------------------------------------------------------------------
uint8_t packetcount(){
	
	double no=(double)packetsize/payloadsize;
	
	no=no*10;
	
	double txtime=(double)packetbits/PHY;	
...
static bool channels_disabled = false;
static void set_data_channel_map() ;
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    // Skip processing if channels are off
    if (channels_disabled) {
        ESP_LOGW(GATTS_TABLE_TAG, "All channels are off; data transmission is disabled.");
        return;  // Return early and skip event handling
    }
    ...
    switch (event) {
        ...
        case ESP_GATTS_WRITE_EVT:
            // Skip the time calculation and logging if channels are disabled
            if (channels_disabled) {
                ESP_LOGW(GATTS_TABLE_TAG, "All channels are off; data transmission is disabled.");
                return;  // Skip further processing if channels are off
            }

            // Only start the timer if channels are enabled
            start_time = clock();  // Start timing
            
            res = find_char_and_desr_index(p_data->write.handle);
            set_data_channel_map(); // Check and possibly update channel map

            if (p_data->write.is_prep == false) {
                ESP_LOGI(GATTS_TABLE_TAG, "ESP_GATTS_WRITE_EVT : handle = %d", res);

                // Time calculation happens only if channels are enabled
                end_time = clock();  // End timing
                time_taken = ((double)(end_time - start_time)) / CLOCKS_PER_SEC * 1000;  // Convert to milliseconds
				if(!channels_disabled)
				{
                ESP_LOGI(GATTS_TABLE_TAG, "Time taken: %.2f ms\n", time_taken);  // Log the time only if channels are enabled
}
        ...
        case ESP_GATTS_READ_EVT:
            if (channels_disabled) return;  // Skip if channels are off

            res = find_char_and_desr_index(p_data->read.handle);
            if (res == SPP_IDX_SPP_STATUS_VAL) {
                // Handle client read of the status characteristic here
            }
            break;
...
// Add a global flag to track whether data transmission is allowed

static void set_data_channel_map() {
    uint8_t channel_map[5] = {0};  // Assuming this is how you set the channel map
    int i;

    
    channel_map[0] = 0; // Channels 0-7 enabled (e.g., 0b11111111)
    channel_map[1] = 0;
    channel_map[2] = 0;

    // Check if all channels are off
    bool all_channels_off = true;
    for (i = 0; i < 5; i++) {
        if (channel_map[i] != 0) {
    ...
    // Set the global flag based on the channel status
    channels_disabled = all_channels_off;
    ...
    // Set the data channel map using ESP BLE API
    esp_err_t ret = esp_gap_ble_set_channels(channel_map);
    if (ret == ESP_OK) {
        ESP_LOGI(GATTS_TABLE_TAG, "Data channel map set successfully to selected channels");
        ESP_LOGI(GATTS_TABLE_TAG, "Channel map: %02X %02X %02X %02X %02X",
            channel_map[0], channel_map[1], channel_map[2], channel_map[3], channel_map[4]);
    } else {
        ESP_LOGE(GATTS_TABLE_TAG, "Failed to set data channel map: %s", esp_err_to_name(ret));
    }
}

    // Calculate time required to hop through channels
  //  double timehop = chcount * 30e-3;
   // ESP_LOGI(GATTS_TABLE_TAG, "Time required to hop through %d channels is %f seconds", chcount, timehop);
