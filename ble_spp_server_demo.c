...
static void set_data_channel_map() {
    ...
    // Enable only channels 0-19 (bits 0-19 set to 1)
    channel_map[0] = 0xFF; // Channels 0-7 enabled (0b11111111)
   // channel_map[1] = 0xFF; // Channels 8-15 enabled (0b11111111)
  // channel_map[2] = 0xFF;
   // channel_map[3]=0xFF;// Channels 16-19 enabled, rest disabled (0b00001111)

    // Set the data channel map using ESP BLE API
    esp_err_t ret = esp_gap_ble_set_channels(channel_map);
    if (ret == ESP_OK) {
        ESP_LOGI(GATTS_TABLE_TAG, "Data channel map set successfully to channels");

        // Print the enabled channels
